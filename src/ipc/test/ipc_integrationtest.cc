...
class MockGreeterService : public ::ipc_test::gen::Greeter {
 public:
  MOCK_METHOD(void, OnSayHello, (const GreeterRequestMsg&, DeferredGreeterReplyMsg*), (override));
  void SayHello(const GreeterRequestMsg& request,
                DeferredGreeterReplyMsg reply) override {
    OnSayHello(request, &reply);
  }

  MOCK_METHOD(void, OnWaveGoodbye, (const GreeterRequestMsg&, DeferredGreeterReplyMsg*), (override));
